...
type Env = {
  Bindings: CloudflareBindings // generated by `npm run cf-typegen` — see "Environment (Cloudflare Workers)"
  Variables: { user: User }
}
...
Bindings (KV, D1, R2, ...) are declared in the wrangler config. Generate their types; never write the `Bindings` type by hand, and rerun after every change to the wrangler config. Projects created with `create-hono` have a script for it:

```bash
npm run cf-typegen   # = wrangler types --env-interface CloudflareBindings
```

This writes `worker-configuration.d.ts`, which declares `CloudflareBindings` together with the Workers runtime types. `@cloudflare/workers-types` is superseded by it: do not install it. If there is no `cf-typegen` script, run `npx wrangler types` and use the interface it declares instead (`Env`, also available as `Cloudflare.Env`).
